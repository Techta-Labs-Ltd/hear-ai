...
def _narrow_to_minimal_diff(
    ...
    """Narrow a segment to only the words that actually differ.

    Strips the common prefix and suffix from both original and edited text,
    adjusting the word index range so only truly changed words are regenerated.
    Returns None if all words match (no actual change).
    """
    orig_list = original_text.split()
    edit_list = edited_text.split()

    # Common prefix
    prefix_len = 0
    for a, b in zip(orig_list, edit_list):
        if _strip_word(a) == _strip_word(b):
            prefix_len += 1
        else:
            break
    ...
    # Common suffix
    suffix_len = 0
    for a, b in zip(reversed(orig_list[prefix_len:]), reversed(edit_list[prefix_len:])):
        if _strip_word(a) == _strip_word(b):
            suffix_len += 1
        else:
            break

    new_start = start_idx + prefix_len
    new_end = end_idx - suffix_len

    if new_start >= new_end:
        return None

    new_original = " ".join(w.word for w in orig_words[new_start:new_end])
    new_edited = " ".join(edit_list[prefix_len:len(edit_list) - suffix_len])

    return new_start, new_end, new_original, new_edited
