...
def _generate_wav_bytes(
    duration_s: float = 1.0, sample_rate: int = 24000, frequency: float = 440.0
) -> bytes:
...
def temp_profiles_dir(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "hear.services.reconstruction.voice_profile.VoiceProfileStore._profiles_root",
        lambda: profiles_dir,
    )
    monkeypatch.setattr(
        "hear.services.reconstruction.voice_profile.VoiceProfileStore._profile_dir",
        lambda user_id: (
...
class TestSaveProfile:
    def test_creates_files(self, temp_profiles_dir):
        ...
        pdir = VoiceProfileStore.save_profile("user-1", wav, "Hello world", language="en")
    ...
    def test_writes_transcript(self, temp_profiles_dir):
        wav = _generate_wav_bytes()
        VoiceProfileStore.save_profile("user-1", wav, "Hello world test")
        text = VoiceProfileStore.get_reference_text("user-1")
    ...
    def test_metadata(self, temp_profiles_dir):
        ...
        VoiceProfileStore.save_profile("user-1", wav, "test", language="en")
    ...
    def test_updates_existing(self, temp_profiles_dir):
        ...
        VoiceProfileStore.save_profile("user-1", wav1, "first text")
        wav2 = _generate_wav_bytes()
        VoiceProfileStore.save_profile("user-1", wav2, "second text")
        text = VoiceProfileStore.get_reference_text("user-1")
...
class TestGetReferencePath:
    def test_returns_path_when_exists(self, temp_profiles_dir):
        wav = _generate_wav_bytes()
        VoiceProfileStore.save_profile("user-1", wav, "test")
        path = VoiceProfileStore.get_reference_path("user-1")
        assert path is not None
        assert path.endswith(".wav")
        assert os.path.isfile(path)

    def test_returns_none_when_missing(self, temp_profiles_dir):
        assert VoiceProfileStore.get_reference_path("nonexistent") is None
...
class TestGetOrCreateFromTrack:
    def test_creates_from_track(self, temp_profiles_dir):
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
            ...
            tmp_path = tmp.name
        try:
            pdir = VoiceProfileStore.get_or_create_from_track(
                "user-1", tmp_path, transcript="track transcript"
            )
    ...
    def test_reuses_existing(self, temp_profiles_dir):
        wav = _generate_wav_bytes()
        VoiceProfileStore.save_profile("user-1", wav, "first")
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
            tmp.write(_generate_wav_bytes())
            tmp_path = tmp.name
        try:
            pdir = VoiceProfileStore.get_or_create_from_track(
                "user-1", tmp_path, transcript="second"
            )
            text = VoiceProfileStore.get_reference_text("user-1")
            assert text == "first"
        finally:
...
class TestListProfiles:
    def test_empty(self, temp_profiles_dir):
        assert VoiceProfileStore.list_profiles() == []

    def test_lists_multiple(self, temp_profiles_dir):
        VoiceProfileStore.save_profile("user-1", _generate_wav_bytes(), "text 1")
        VoiceProfileStore.save_profile("user-2", _generate_wav_bytes(), "text 2")
        profiles = VoiceProfileStore.list_profiles()
...
class TestCleanupProfiles:
    def test_nothing_to_clean(self, temp_profiles_dir):
        VoiceProfileStore.save_profile("user-1", _generate_wav_bytes(), "test")
        removed = VoiceProfileStore.cleanup_profiles(max_age_hours=168)
        assert removed == 0

    def test_removes_old(self, temp_profiles_dir, monkeypatch):
        VoiceProfileStore.save_profile("old-user", _generate_wav_bytes(), "test")
        ...
        removed = VoiceProfileStore.cleanup_profiles(max_age_hours=168)
        assert removed == 1
        assert VoiceProfileStore.get_reference_path("old-user") is None
