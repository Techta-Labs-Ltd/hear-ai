...
from hear.services.magic_clean.processing.validation import (
    AudioValidationError,
    mp3_duration_tolerance_seconds,
...
def _write_wav(path: Path, audio: np.ndarray, sample_rate: int = 44_100) -> None:
...
def test_validate_pcm_rejects_invalid_shape_empty_and_nonfinite(
    waveform: torch.Tensor,
) -> None:
    with pytest.raises(AudioValidationError):
        validate_pcm(waveform, expected_samples=None, preserve_timeline=False)


def test_validate_pcm_requires_and_enforces_exact_preserved_length() -> None:
    waveform = torch.zeros((2, 100), dtype=torch.float32)

    validate_pcm(waveform, expected_samples=100, preserve_timeline=True)
    validate_pcm(waveform, expected_samples=None, preserve_timeline=False)

    with pytest.raises(AudioValidationError, match="Expected PCM sample count"):
        validate_pcm(waveform, expected_samples=None, preserve_timeline=True)
    with pytest.raises(AudioValidationError, match="expected=99, actual=100"):
        validate_pcm(waveform, expected_samples=99, preserve_timeline=True)


def test_mp3_duration_allowance_is_one_frame_or_30ms_and_never_proportional() -> None:
    assert mp3_duration_tolerance_seconds(48_000) == pytest.approx(0.030)
    assert mp3_duration_tolerance_seconds(44_100) == pytest.approx(0.030)
    assert mp3_duration_tolerance_seconds(32_000) == pytest.approx(0.036)
    assert mp3_duration_tolerance_seconds(16_000) == pytest.approx(0.036)
    with pytest.raises(ValueError, match="sample rate"):
        mp3_duration_tolerance_seconds(0)


def test_unavailable_snr_sentinel_is_excluded_from_quality_score() -> None:
    metrics = QualityMetrics()

    unavailable = metrics.compute_quality_score(
        0.0,
        False,
        -16.0,
        snr_available=False,
    )
    treated_as_measurement = metrics.compute_quality_score(
        0.0,
        False,
        -16.0,
        snr_available=True,
...
def test_delivered_mp3_returns_measurements_from_exact_decoded_artifact(
    ...
    result = validate_delivered_audio(
        str(source_path),
        str(output_path),
        cut_silence=False,
        expect_audible=True,
    ...
    assert result.duration_delta_seconds <= mp3_duration_tolerance_seconds(
        decoded_sample_rate
    )
...
def test_silence_edit_allows_delivery_resampling_rounding(
    tmp_path: Path,
    ffmpeg_path: str,
) -> None:
    source_rate = 96_000
    ...
    source = _tone(
        ...
        duration_seconds=source_samples / source_rate,
    )
...
def test_silence_edit_still_rejects_a_lengthened_retained_pcm_timeline(
    ...
    source = torch.full((1, 48_000), 0.1)
    lengthened = torch.full((1, 48_001), 0.1)
    decoded = {
        "source": source,
        "reference": lengthened,
        "output": lengthened,
    }
    monkeypatch.setattr(AudioIO, "load", lambda path: (decoded[path], 48_000))
...
def test_delivered_mp3_rejects_missing_source_channel(
    tmp_path: Path,
    ffmpeg_path: str,
) -> None:
...
def test_delivered_mp3_rejects_erased_audible_stereo_channel(
    ...
    with pytest.raises(
        AudioValidationError,
        match=r"audible source channel: channel=2",
    ):
        validate_delivered_audio(
            str(source_path),
            str(output_path),
...
def test_silent_source_and_delivery_are_valid_and_use_unavailable_sentinels(
    ...
    _write_wav(source_path, np.zeros(44_100, dtype=np.float32))
    _encode_mp3(ffmpeg_path, source_path, output_path)
...
def test_silent_source_cannot_turn_into_audible_delivery(
    tmp_path: Path,
    ffmpeg_path: str,
) -> None:
...
@pytest.mark.parametrize("sample_count", [19_199, 19_200])
def test_sub_r128_window_clip_uses_finite_unavailable_loudness(
    ...
    _write_wav(
        source_path,
        _tone(sample_rate=sample_rate, duration_seconds=duration_seconds),
        sample_rate,
    )
    _encode_mp3(ffmpeg_path, source_path, output_path)
...
def test_silence_edit_allows_removing_real_silence(
    tmp_path: Path,
    ffmpeg_path: str,
) -> None:
    ...
    source = np.zeros(441_000, dtype=np.float32)
    source[176_400:220_500] = _tone(duration_seconds=1.0)
...
def test_intentionally_zero_delivery_is_allowed_but_unexpected_silence_fails(
    ...
    _write_wav(silent_path, np.zeros(44_100, dtype=np.float32))
...
def test_injected_decoded_artifact_rejects_erased_audible_channel(
    ...
    source = torch.full((2, 48_000), 0.1)
    delivered = source.clone()
    delivered[1].zero_()
    monkeypatch.setattr(
        AudioIO,
        "load",
        lambda path: (source, 48_000) if path == "source" else (delivered, 48_000),
    )
...
def test_channel_peak_cannot_hide_near_total_rms_erasure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source = torch.full((2, 48_000), 0.1)
    delivered = source.clone()
...
@pytest.mark.parametrize(
    "source",
    [
        torch.full((1, 48_000), 5e-7),
        torch.nn.functional.pad(torch.tensor([[2e-5]]), (0, 47_999)),
    ],
...
def test_channel_collapse_gate_accepts_identity_at_single_metric_floor(
    ...
    monkeypatch.setattr(AudioIO, "load", lambda _path: (source, 48_000))
...
def test_decoded_artifact_rejects_true_peak_above_minus_one(
    ...
    source = torch.full((1, 48_000), 0.9)
    delivered = source.clone()
    monkeypatch.setattr(AudioIO, "load", lambda _path: (delivered, 48_000))
