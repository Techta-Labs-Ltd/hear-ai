...
def test_extract_enhanced_audio_url_reads_only_exact_nested_field():
    assert (
        MagicCleanLineageResolver.extract_enhanced_audio_url(
    ...
    assert (
        MagicCleanLineageResolver.extract_enhanced_audio_url(
            {"enhanced_url": "https://audio/wrong.mp3"}
        )
        == ""
    )
    assert (
        MagicCleanLineageResolver.extract_enhanced_audio_url(
            {"enhanced_audio": "https://audio/wrong.mp3"}
        )
        == ""
    )
    assert MagicCleanLineageResolver.extract_enhanced_audio_url(None) == ""
...
def test_five_exact_derivative_runs_resolve_to_one_canonical_source():
    ...
    for index in range(1, 6):
        ...
        jobs.append(_job(f"job-{index}", input_url=current_url, output_url=output_url))
        current_url = output_url
    resolution = MagicCleanLineageResolver.resolve_magic_clean_lineage(
        current_url, backend_id="backend-a", track_id="track-a", jobs=jobs
    )
    ...
    assert resolution.lineage_job_ids == ("job-5", "job-4", "job-3", "job-2", "job-1")
...
def test_persisted_root_hint_short_circuits_legacy_parent_chain():
    ...
    resolution = MagicCleanLineageResolver.resolve_magic_clean_lineage(
        "https://audio/output.mp3", backend_id="backend-a", track_id="track-a", jobs=[job]
    )
...
def test_unknown_or_cross_scope_exact_output_is_a_new_source(candidate):
    ...
    resolution = MagicCleanLineageResolver.resolve_magic_clean_lineage(
        submitted_url, backend_id="backend-a", track_id="track-a", jobs=[candidate]
    )
...
def test_exact_url_ambiguity_fails_closed():
    jobs = [
        _job("job-1", input_url="https://audio/root-a.wav", output_url="https://audio/shared.mp3"),
        _job("job-2", input_url="https://audio/root-b.wav", output_url="https://audio/shared.mp3"),
    ]
    with pytest.raises(MagicCleanLineageError, match="ambiguous"):
        MagicCleanLineageResolver.resolve_magic_clean_lineage(
            "https://audio/shared.mp3", backend_id="backend-a", track_id="track-a", jobs=jobs
        )
...
def test_cycle_and_excessive_depth_fail_closed():
    cyclic_jobs = [
        _job(
            "job-a", input_url="https://audio/output-b.mp3", output_url="https://audio/output-a.mp3"
        ),
        _job(
            "job-b", input_url="https://audio/output-a.mp3", output_url="https://audio/output-b.mp3"
        ),
    ]
    ...
    deep_jobs = [
        _job("job-1", input_url="https://audio/root.wav", output_url="https://audio/output-1.mp3"),
        _job(
            "job-2", input_url="https://audio/output-1.mp3", output_url="https://audio/output-2.mp3"
        ),
    ]
...
def test_known_output_without_parent_or_root_fails_closed():
    job = _job("job-1", input_url="", output_url="https://audio/output.mp3")
...
@pytest.mark.parametrize(
    ...
    [
        ("delivered_file_sha256", "submitted_file_sha256", "delivered_file_sha256"),
        ("delivered_pcm_sha256", "submitted_pcm_sha256", "delivered_pcm_sha256"),
    ],
)
def test_hash_alias_resolves_prior_output_to_root(hash_field, argument_name, expected_method):
    ...
    resolution = MagicCleanLineageResolver.resolve_magic_clean_hash_alias(
...
def test_multiple_hash_matches_are_safe_only_when_their_roots_agree():
    ...
    with pytest.raises(MagicCleanLineageError, match="ambiguous"):
        MagicCleanLineageResolver.resolve_magic_clean_hash_alias(
            "https://audio/reupload.mp3",
            backend_id="backend-a",
            track_id="track-a",
...
def test_cross_scope_hash_and_unrecognized_transcode_are_new_sources():
    ...
    cross_scope = MagicCleanLineageResolver.resolve_magic_clean_hash_alias(
        "https://audio/reupload.mp3",
        backend_id="backend-a",
        track_id="track-a",
        jobs=[job],
    ...
    transcoded = MagicCleanLineageResolver.resolve_magic_clean_hash_alias(
...
def test_file_sha256_streams_exact_bytes(tmp_path):
    ...
    assert (
        MagicCleanLineageResolver.sha256_file(path, chunk_size=17)
        == hashlib.sha256(content).hexdigest()
    )
    with pytest.raises(ValueError, match="chunk_size"):
        MagicCleanLineageResolver.sha256_file(path, chunk_size=0)
...
def test_decoded_pcm_hash_is_container_independent_and_repeatable(tmp_path):
    ...
    wav_hash = MagicCleanLineageResolver.sha256_decoded_pcm(wav_path, chunk_size=31)
    flac_hash = MagicCleanLineageResolver.sha256_decoded_pcm(flac_path, chunk_size=47)
    assert MagicCleanLineageResolver.sha256_file(wav_path) != MagicCleanLineageResolver.sha256_file(
        flac_path
    )
    assert wav_hash == flac_hash
    assert wav_hash == MagicCleanLineageResolver.sha256_decoded_pcm(wav_path)
    with pytest.raises(ValueError, match="timeout_seconds"):
        MagicCleanLineageResolver.sha256_decoded_pcm(wav_path, timeout_seconds=0)
...
def test_decoded_pcm_hash_header_distinguishes_rate_and_channel_layout(tmp_path):
    ...
    sf.write(rate_a, sample_values, 8000, subtype="PCM_16")
    sf.write(rate_b, sample_values, 16000, subtype="PCM_16")
    sf.write(mono, sample_values, 8000, subtype="PCM_16")
    sf.write(stereo, sample_values.reshape(1, 2), 8000, subtype="PCM_16")
    assert MagicCleanLineageResolver.sha256_decoded_pcm(
        rate_a
    ) != MagicCleanLineageResolver.sha256_decoded_pcm(rate_b)
    assert MagicCleanLineageResolver.sha256_decoded_pcm(
        mono
    ) != MagicCleanLineageResolver.sha256_decoded_pcm(stereo)
