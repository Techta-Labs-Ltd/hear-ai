...
def test_magic_clean_exact_replay_survives_credential_ttl_erosion(monkeypatch):
    ...
    monkeypatch.setattr(
        "hear.services.jobs.submission.SessionLocal",
        lambda: db,
    )
    monkeypatch.setattr(
        "hear.services.jobs.submission.encrypt_storage_context",
        lambda _storage: "encrypted-short-lived-context",
    )
    monkeypatch.setattr(
        "hear.services.jobs.submission.decrypt_storage_context",
...
def test_terminal_magic_clean_replay_renews_cleanup_credentials(
    monkeypatch,
    status,
):
    ...
    refreshed_payload = normalize_request(refreshed_request)
    tombstone = {
        "b2_key": original_request.storage.folder_prefix
        + "enhanced/clean.mp3",
    ...
    assert job.job_options["storage_destination"]["expires_at"] == (
        refreshed_payload["storage"]["expires_at"]
    )
    assert job.job_options["magic_clean_cleanup_tombstone"] == tombstone
    assert job.request_hash == request_fingerprint(refreshed_payload)
...
def test_terminal_magic_clean_cleanup_credential_renewal_must_advance_expiry(
    ...
    submitted_storage = {
        **original_storage,
        "expires_at": (
            original_storage["expires_at"]
            if rotate_key
            else (datetime.now(UTC) + timedelta(days=3)).isoformat()
        ),
    }
    if rotate_key:
        submitted_storage.update(
            key_id="rotated-key-id",
            application_key="rotated-application-key",
        )
...
def test_new_magic_clean_with_short_credentials_rolls_back_before_commit(
    monkeypatch,
):
    short_lived_storage = {
        **STORAGE,
...
def test_magic_clean_replay_can_refresh_nonterminal_credentials(monkeypatch):
    ...
    original_payload = normalize_request(original_request)
    refreshed_storage = {
        **STORAGE,
        "key_id": "rotated-key-id",
        "application_key": "rotated-application-key",
    ...
    job = SimpleNamespace(
        ...
        job_type="magic_clean",
        status="queued",
        request_hash=request_fingerprint(original_payload),
        job_options=_job_options(original_payload),
...
def test_magic_clean_delayed_credential_retry_cannot_regress_refresh(
    monkeypatch,
):
    original_request = pipeline_request(
        job_id="clean",
...
def test_magic_clean_rejects_unsafe_nonterminal_credential_refresh(monkeypatch):
    ...
    original_payload = normalize_request(
        original_request,
        enforce_magic_clean_storage_ttl=False,
    )
    short_lived_storage = {
...
def test_magic_clean_key_rotation_requires_later_expiration(monkeypatch):
    ...
    payload = normalize_request(original_request)
    ...
    rotated_payload = normalize_request(rotated_request)
    assert request_fingerprint(rotated_payload) == request_fingerprint(payload)
...
def test_magic_clean_records_omitted_controls_without_changing_idempotency():
    ...
    explicit_default = normalize_request(
        pipeline_request(
            job_id="job",
            track_id="track",
            user_id="user",
    ...
    assert request_fingerprint(omitted) == request_fingerprint(explicit_default)
