...
from hear.services.magic_clean.streaming import (
    MagicCleanProcessingCancelled,
    _decode_lossless,
...
class IdentityPipeline:
    ...
    def finalise(
        ...
        *,
        ...
        # A deterministic stand-in for one global edit. If this were applied
        # per processing window, the resulting duration would be much shorter.
...
def _write_stereo_fixture(path, *, sample_rate: int = 48_000, seconds: float = 1.25):
...
@pytest.mark.parametrize("chunk_seconds", [0.30, 60.0])
def test_disk_backed_engine_preserves_stereo_timeline_and_validates_mp3(
    tmp_path,
    chunk_seconds,
):
    ...
    delivered = validate_delivered_audio(
        str(source_path),
        str(output_path),
        cut_silence=False,
        expect_audible=True,
...
def test_lossless_decode_enables_automatic_rf64_upgrade(monkeypatch):
    ...
    monkeypatch.setattr(
        "hear.services.magic_clean.streaming._run",
        lambda command: commands.append(command),
    )

    _decode_lossless("source.audio", "decoded.wav")
...
def test_high_sample_rate_source_uses_supported_mp3_delivery_rate(tmp_path):
    ...
    output_path = tmp_path / "cleaned.mp3"
    _write_stereo_fixture(source_path, sample_rate=96_000, seconds=0.6)
    ...
    assert result.sample_rate == 96_000
    assert result.delivery_sample_rate == 48_000
    assert decoded_rate == 48_000
...
def test_limiter_compensates_lookahead_without_shifting_or_losing_tail(tmp_path):
    ...
    assert int(absolute[:1_000].argmax()) == pytest.approx(100, abs=2)
    tail_peak = int(absolute[-1_000:].argmax()) + decoded.shape[1] - 1_000
...
def test_codec_true_peak_overshoot_is_corrected_before_delivery(tmp_path):
    ...
    burst_samples = round(0.010 * sample_rate)
