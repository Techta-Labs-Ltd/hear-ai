...
def test_audio_tag_result_contains_only_transcription_output():
    ...
    result = OrchestrationResults.audio_tag_result(
        job, track, "Tag this as construction news.", ["#construction", "#news", "#ignored"]
    )
    assert result == {
        "job_id": "job-1",
        "run_id": "run-1",
...
async def _assert_audio_tag_pipeline_stops_after_transcription(monkeypatch, tmp_path):
    ...
    transcription = {"transcript": "A short audio clip.", "segments": [], "language": "en"}
    job = SimpleNamespace(
        id="job-1",
        run_id="run-1",
        backend_id="backend-a",
    ...
    monkeypatch.setattr("hear.orchestrator.DatabaseCommitter.commit_with_retry", AsyncMock())
    ...
    orchestrator._transcriber.transcribe_file.assert_awaited_once_with(
        str(audio_path), job_id="job-1", run_id="run-1", track_id="track-1", short_utterance=True
    )
