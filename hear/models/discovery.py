...
class DiscoverySerialization:
    ...
    @staticmethod
    def content_description_from_discovery(
    ...
    def discovery_to_callback_dict(
        ...
        *,
        ...
        return {
            ...
            "source": DiscoverySerialization.coerce_discovery_source(source, profile.source),
            "duration_seconds": int(dur) if dur is not None else 0,
            "audience_relevance": audience,
            "tone": list(profile.tone or []),
            "entities": entities_obj,
            "entities_flat": DiscoverySerialization.flatten_entities(profile.entities),
            "key_themes": themes,
...
class DiscoveryEntities(BaseModel):
    ...
    def is_empty(self) -> bool:
        return not any((self.people, self.animals, self.products, self.apps, self.technologies))
