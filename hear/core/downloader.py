    ...
    try:
        async with httpx.AsyncClient(
            ...
            async with client.stream("GET", url) as response:
                ...
                if expected_length is not None and int(expected_length) > settings.AUDIO_DOWNLOAD_MAX_BYTES:
        ...
        if convert_to_wav:
            await run_blocking_to_completion(
                lambda: _convert_to_wav(download_path, path, preserve_channels=preserve_channels)
            )
...
def _convert_to_wav(source_path: str, wav_path: str, *, preserve_channels: bool = False) -> None:
    """Decode a downloaded audio file into a real mono PCM WAV file."""
    subprocess.run(
        [
            "ffmpeg", "-nostdin", "-v", "error", "-y", "-i", source_path,
            "-vn", *([] if preserve_channels else ["-ac", "1"]),
            "-c:a", "pcm_s16le", "-rf64", "auto", wav_path,
        ],
        capture_output=True,
        check=True,
