...
class TempWorkspace:
    ...
    def hear_temp_managed_directory() -> str:
        path = os.path.join(TempWorkspace.hear_temp_directory(), "managed")
        os.makedirs(path, exist_ok=True)
        return path
    ...
    def hear_temp_job_dir(job_id: str, run_id: str) -> str:
        path = os.path.join(
            TempWorkspace.hear_temp_managed_directory(),
            "jobs",
            TempWorkspace._safe_component(job_id),
            TempWorkspace._safe_component(run_id),
        )
        os.makedirs(path, exist_ok=True)
        return path
    ...
    def hear_temp_standalone_dir(purpose: str = "audio") -> str:
        ...
        root = os.path.join(TempWorkspace.hear_temp_managed_directory(), "standalone")
    ...
    def drop_temp_standalone(path: str) -> None:
        ...
        try:
            if os.path.isfile(path):
                ...
                standalone_root = os.path.join(
                    TempWorkspace.hear_temp_managed_directory(), "standalone"
                )
    ...
    def sweep_tracked_temp_files() -> dict:
        root = TempWorkspace.hear_temp_managed_directory()
            ...
            for entry in os.scandir(category_path):
                try:
                    if entry.stat(follow_symlinks=False).st_mtime >= cutoff:
                        continue
                    size = TempWorkspace._path_size(entry.path)
                    ...
                    continue
        temp_root = TempWorkspace.hear_temp_directory()
    ...
    def purge_all_temp() -> dict:
        ...
        for entry in os.scandir(root):
            try:
                bytes_freed += TempWorkspace._path_size(entry.path)
                if entry.is_dir(follow_symlinks=False):
                    shutil.rmtree(entry.path, ignore_errors=True)
                else:
                    os.unlink(entry.path)
