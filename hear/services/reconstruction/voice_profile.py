...
class VoiceProfileStore:
    ...
    def _profiles_root() -> str:
        ...
        if custom:
            ...
            path = os.path.join(TempWorkspace.hear_temp_directory(), "voice_profiles")
    ...
    def _profile_dir(user_id: str) -> str:
        ...
        path = os.path.join(VoiceProfileStore._profiles_root(), safe_id)
        os.makedirs(path, exist_ok=True)
        return path
    ...
    def save_profile(
        ...
        *,
        ...
        """
        pdir = VoiceProfileStore._profile_dir(user_id)
            ...
            try:
                ...
                try:
                    with wave.open(ref_wav, "rb") as wav_file:
                        actual_duration = round(wav_file.getnframes() / wav_file.getframerate(), 3)
                except (OSError, EOFError, wave.Error, ZeroDivisionError):
                    actual_duration = 0.0
        meta = VoiceProfileStore._read_metadata(pdir)
        meta.update(
            {
                "user_id": user_id,
        ...
        VoiceProfileStore._write_metadata(pdir, meta)
        return pdir
    ...
    def get_or_create_from_track(
        user_id: str, audio_path: str, *, transcript: Optional[str] = None
    ) -> str:
        ...
        str
            The profile directory path.
        """
        existing = VoiceProfileStore.get_reference_path(user_id)
        if existing:
            return VoiceProfileStore._profile_dir(user_id)
        with open(audio_path, "rb") as f:
            audio_bytes = f.read()
        return VoiceProfileStore.save_profile(user_id, audio_bytes, transcript=transcript or "")

    @staticmethod
    def list_profiles() -> list[dict]:
        """Return metadata for all stored voice profiles."""
        root = VoiceProfileStore._profiles_root()
        profiles: list[dict] = []
    ...
    def cleanup_profiles(max_age_hours: Optional[int] = None) -> int:
        ...
        """
        max_hours = (
            max_age_hours if max_age_hours is not None else settings.VOICE_PROFILE_MAX_AGE_HOURS
        )
