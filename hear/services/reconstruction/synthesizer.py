...
class SpeechSynthesizer:
    ...
    def _compute_seed(self, _job_id: str, track_id: str) -> int:
        ...
        digest = hashlib.sha256(f"reconstruction:{track_id}".encode()).digest()
        return int.from_bytes(digest[:4], "big") & 2147483647

    @staticmethod
    def _analyze_prosody(
        waveform: torch.Tensor, sr: int, start_sample: int, end_sample: int
    ) -> dict:
        ...
        rms_variation = (
            float(np.std(frame_rms) / max(np.mean(frame_rms), 1e-10)) if len(frame_rms) > 1 else 0.0
        )
        ...
        logger.debug(
            "Prosody analysis: energy=%.1fdB rms_var=%.2f rate=%.1fw/s -> temp=%s top_p=%s chunk=%s",
            energy_db,
            rms_variation,
            speaking_rate,
    ...
    async def reconstruct_segment(
        ...
        reconstructed = self._splice_segment(
    ...
    async def reconstruct_segments(
        ...
        if voice_reference_audio_path and voice_reference_audio_path != original_audio_path:
            reference_waveform, reference_sr = torchaudio.load(voice_reference_audio_path)
            if reference_sr != self.TARGET_SR:
                reference_waveform = F_audio.resample(
                    reference_waveform, reference_sr, self.TARGET_SR
                )
        ...
        for change in normalized:
            timeline_start = int(float(change["segment_start"]) * self.TARGET_SR)
            timeline_end = int(float(change["segment_end"]) * self.TARGET_SR)
            start_sample = max(
                0, min(timeline_start, merged.shape[1] - 1 if merged.shape[1] else 0)
            )
            end_sample = max(start_sample + 1, min(timeline_end, merged.shape[1]))
            reference_start = max(0, min(timeline_start, reference_waveform.shape[1]))
            reference_end = max(reference_start, min(timeline_end, reference_waveform.shape[1]))
            if change.get("is_deletion"):
                _recon_payload_logger.info(
                    "DELETE | seg=%.1fs-%.1fs | track=%s",
                    change["segment_start"],
                    change["segment_end"],
                    track_id,
                )
                merged = self._splice_segment(
                    merged, torch.zeros_like(merged[:, :0]), start_sample, end_sample
                )
            ...
            if ref_segment.shape[1] < int(self.TARGET_SR * 1.0):
                ...
                ref_end = min(reference_waveform.shape[1], reference_end + ref_radius)
            ...
            logger.info(
                ...
                orig_seg_dur,
                tts_seg_dur,
                tts_seg_dur - orig_seg_dur,
                change["new_text"][:50],
            )
            segment_results.append(
                await self._upload_segment_audio(
                    tts_waveform,
                    track_id=track_id,
                    segment_start=float(change["segment_start"]),
                    segment_end=float(change["segment_end"]),
    ...
    async def reconstruct_segments_batched(
        ...
        for batch_idx, batch in enumerate(batches):
            ...
            for change in batch_sorted:
                ...
                start_sample = max(
                    0, min(start_sample, merged.shape[1] - 1 if merged.shape[1] else 0)
                )
    ...
    async def remove_segment(
        ...
        if cross_len > 0 and before.shape[1] > 0 and (after.shape[1] > 0):
            ...
            before[:, -cross_len:] = (
                before[:, -cross_len:] * fade_out + after[:, :cross_len] * fade_in
            )
    ...
    def _split_into_batches(self, changes: list[dict], waveform: torch.Tensor) -> list[list[dict]]:
    ...
    async def _synthesize_fishspeech(
        ...
        _recon_payload_logger.info(
            "FISHSPEECH_REQ | text='%s' | processed='%s'", text[:120], processed_text[:120]
        )
        refs = None
        ...
        return await self._model_client.generate_speech(
            text=processed_text, max_new_tokens=1024, references=refs, seed=seed
        )

    async def _preprocess_for_s2(self, text: str) -> str:
        """Preserve the requested delivery and mark only explicit paragraph breaks."""
        paragraphs = re.split("\\n\\s*\\n", text.strip())
        normalized = [re.sub("\\s+", " ", paragraph).strip() for paragraph in paragraphs]
    ...
    async def _generate_segment_groups(
        ...
        return await self._synthesize_fishspeech(
            text, reference_audio_path, original_text, seed=seed, emotion_params=emotion_params
        )
    ...
    def _reference_clip_bounds(
        self, waveform: torch.Tensor, start_sample: int, end_sample: int
    ) -> tuple[int, int]:
    ...
    @classmethod
    def _speaking_rate_from_transcription(cls, transcription: dict) -> float | None:
        ...
        if not (
            ...
            and cls.MIN_PLAUSIBLE_SPEAKING_RATE <= speaking_rate <= cls.MAX_PLAUSIBLE_SPEAKING_RATE
        ):
            return None
    ...
    def _transcription_text_for_window(
        cls, transcription: dict, window_start: float, window_end: float
    ) -> str:
        ...
        selected_words = [
            ...
            if word_units <= 2 and window_start <= (word_start + word_end) / 2 < window_end
        ]
        if selected_words:
    ...
    def _complete_word_reference_window(
        ...
        return (
            selected[0][0],
            selected[-1][1],
            " ".join((word_text for _start, _end, word_text in selected)),
        )
    ...
    async def _prepare_voice_reference(
        ...
        *,
        ...
        minimum_samples = int(self.MIN_VOICE_REFERENCE_SECONDS * self.TARGET_SR)
        use_edited_interval = (
            self._speech_units(original_text) > 0 and edit_end - edit_start >= minimum_samples
        )
        if use_edited_interval:
            ...
            max_pacing_samples = int(self.MAX_PACING_REFERENCE_SECONDS * self.TARGET_SR)
            pacing_samples = min(max_pacing_samples, edit_end - edit_start)
            pacing_start = edit_start + (edit_end - edit_start - pacing_samples) // 2
            pacing_end = pacing_start + pacing_samples
        else:
            ref_start, ref_end = self._reference_clip_bounds(waveform, edit_start, edit_end)
        ...
        try:
            if use_edited_interval:
                pacing_audio = (
                    waveform[:, pacing_start:pacing_end].detach().float().mean(dim=0).cpu().numpy()
                )
                audio_bytes = await asyncio.to_thread(
                    self._wav_bytes_from_audio, pacing_audio, self.TARGET_SR
                )
            else:
            ...
            if use_edited_interval:
                ...
                if aligned_window is not None:
                    ...
                    aligned_samples = round((aligned_end - aligned_start) * self.TARGET_SR)
                    if aligned_samples >= minimum_samples:
                        ref_start = pacing_start + round(aligned_start * self.TARGET_SR)
            ...
            if speaking_rate is not None:
                logger.warning(
                    "Voice clone reference text was not aligned for track=%s; using measured speaking rate without voice reference",
                    track_id,
                )
                if reference_path:
                    TempWorkspace.drop_temp_standalone(reference_path)
    ...
    def _detect_speech_bounds(
        ...
        threshold = max(float(np.percentile(envelope, 65) * 0.45), 0.0001)
    ...
    def _pacing_reference(
        ...
        *,
        ...
        if self._speech_units(clean_reference_text) > 0:
            ref_start, ref_end = self._reference_clip_bounds(waveform, start_sample, end_sample)
            if ref_end > ref_start:
    ...
    def _speech_units(text: str | None) -> int:
        ...
        without_controls = re.sub("\\[[^\\]]+\\]", " ", str(text or ""))
        return len(re.findall("[^\\W_]+(?:['’][^\\W_]+)?", without_controls, re.UNICODE))

    @classmethod
    def _active_speech_duration(
        cls, waveform: torch.Tensor, sr: int, *, allow_uniform_activity: bool = True
    ) -> float:
        ...
        if frame_count < 2:
            ...
            return len(mono) / sr if peak >= 1e-05 else 0.0
        ...
        if low_energy / high_energy >= 0.65:
            ...
            threshold = max(
                1e-05, high_energy * 0.08, float(np.sqrt(max(low_energy, 0.0) * high_energy))
            )
        ...
        *,
        ...
        if source_rate is None:
            ...
            if source_span_dur < 0.05:
                logger.warning("Speech-rate match skipped because source span is too short")
                return tts_waveform
        ...
        if not (
            ...
            and (
                self.MIN_PLAUSIBLE_SPEAKING_RATE <= source_rate <= self.MAX_PLAUSIBLE_SPEAKING_RATE
            )
            and (
                self.MIN_PLAUSIBLE_SPEAKING_RATE <= raw_tts_rate <= self.MAX_PLAUSIBLE_SPEAKING_RATE
            )
        ):
            logger.warning(
                "Speech-rate match skipped because source/raw rates are unreliable (source=%.3f raw_tts=%.3f)",
                source_rate,
                raw_tts_rate,
            )
            return tts_waveform
        ...
        applied_rate = float(
            np.clip(requested_rate, self.MIN_SAFE_TEMPO_FACTOR, self.MAX_SAFE_TEMPO_FACTOR)
        )
        target_samples = max(1, round(current_samples / applied_rate))
        target_dur = target_samples / self.TARGET_SR
        logger.info(
            "Speech-rate profile: source=%.2f words/s raw_tts=%.2f words/s requested=%.3f applied=%.3f measurement=%s duration=%.2fs->%.2fs",
            source_rate,
            raw_tts_rate,
            requested_rate,
            applied_rate,
        ...
        _recon_payload_logger.info(
            "PACING_PROFILE | source=%.3f words/s | raw_tts=%.3f words/s | requested=%.4f | applied=%.4f | measurement=%s | duration=%.3fs->%.3fs",
            source_rate,
            raw_tts_rate,
            requested_rate,
            applied_rate,
        ...
        if not np.isclose(requested_rate, applied_rate):
            ...
            logger.warning(
                "Speech-rate correction clamped: requested=%.3f applied=%.3f residual=%+.1f%%",
                requested_rate,
                applied_rate,
                residual_percent,
            )
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                stretched, stretched_sr = torchaudio.sox_effects.apply_effects_tensor(
                    sox_input, self.TARGET_SR, effects
                )
            if stretched_sr != self.TARGET_SR:
                stretched = F_audio.resample(stretched, stretched_sr, self.TARGET_SR)
            stretched = stretched.to(device=tts_waveform.device, dtype=tts_waveform.dtype)
            return self._fit_waveform_length(stretched, target_samples)
        except Exception as e:
            logger.warning("Speech tempo matching failed: %s, preserving natural TTS speed", e)
            return tts_waveform
    ...
    def _splice_segment(
        ...
        cross_len = min(int(0.03 * self.TARGET_SR), before.shape[1], replacement_waveform.shape[1])
        if cross_len > 0 and before.shape[1] > 0 and (replacement_waveform.shape[1] > 0):
            fade_out = torch.linspace(1.0, 0.0, cross_len).unsqueeze(0)
            fade_in = torch.linspace(0.0, 1.0, cross_len).unsqueeze(0)
            before[:, -cross_len:] = (
                before[:, -cross_len:] * fade_out + replacement_waveform[:, :cross_len] * fade_in
            )
        ...
        if cross_end > 0 and after.shape[1] > 0:
            ...
            replacement_waveform[:, -cross_end:] = (
                replacement_waveform[:, -cross_end:] * tail_out + after[:, :cross_end] * tail_in
            )
