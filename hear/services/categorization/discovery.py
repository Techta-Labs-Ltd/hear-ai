...
_FILENAME_TITLE = re.compile("^\\d{8}[\\d_\\-a-fA-Z]*$")
_SPEAKER_PATTERNS = (
    re.compile(
        "\\b(?:this is|i am|i'm|my name is)\\s+([A-Z][a-z]+(?:\\s+[A-Z][a-z]+){0,3})", re.IGNORECASE
    ),
    re.compile(
        "\\b(?:with|from)\\s+([A-Z][a-z]+(?:\\s+[A-Z][a-z]+){0,2})\\s+in\\s+[A-Z]", re.IGNORECASE
    ),
)
_NON_PERSON_SPEAKER = re.compile(
    "\\b(?:glasses|hardware|wearable|device|technology|recruitment|production|podcast|accessibility|assistive)\\b",
    re.IGNORECASE,
)
...
_PERSON_NAME_SHAPE = re.compile("^[A-Z][a-z]+(?:\\s+[A-Z][a-z]+){0,3}$")
...
class DiscoverySupport:
    ...
    def discovery_result_bundle(
        ...
        *,
        ...
        return (
            DiscoverySerialization.discovery_to_callback_dict(
                profile,
...
class DiscoveryService:
    ...
    def _is_plausible_speaker(
        self, name: str, transcript: str, *, from_regex: bool = False
    ) -> bool:
        s = (name or "").strip()
        if not s or len(s) < 3:
            return False
        ...
        if not _PERSON_NAME_SHAPE.match(s):
            return False
        intro = re.compile(
            f"\\b(?:this is|i am|i'm|my name is|called|named)\\s+{re.escape(s)}\\b", re.IGNORECASE
        )
        return bool(intro.search(transcript or ""))

    def _sanitize_speaker(
        self, speaker: str | None, transcript: str, *, trusted: bool = False
    ) -> str | None:
        if trusted:
            s = (speaker or "").strip()
    ...
    def _is_weak_profile(
        self, profile: ContentDiscoveryProfile, transcript: str, track_name: str
    ) -> bool:
    ...
    def _enrich_profile(
        ...
        profile.entities.people = [
            ...
            if DiscoverySupport._norm_label(p)
            != DiscoverySupport._norm_label(profile.speaker or "")
        ...
        if (
            not profile.embedding_source_text
            or profile.embedding_source_text.lower()[:60] == tx.lower()[:60]
        ):
        ...
        if not profile.recommendation_labels and profile.primary_genre:
            profile.recommendation_labels = [f"For listeners interested in {profile.primary_genre}"]
        if not profile.one_line_description and profile.summary_short:
            profile.one_line_description = profile.summary_short[:500]
        if not profile.summary_short or self._summary_reads_like_transcript(
            profile.summary_short, tx
        ):
    ...
    def map_freeform_tags(
        self, profile: ContentDiscoveryProfile, categorization: dict | None, *, transcript: str = ""
    ) -> list[str]:
        free: list[str] = []
        seen: set[str] = set()
    ...
    def _finalize_discovery_profile(
        self, profile: ContentDiscoveryProfile, transcript: str
    ) -> ContentDiscoveryProfile:
        tx = (transcript or "").strip()
        profile.speaker = self._sanitize_speaker(profile.speaker, tx)
        ...
        profile.entities.products = [
            ...
            if DiscoverySupport._norm_label(p) not in _NON_PERSON_SPEAKER_NAMES
        ]
        return profile
    ...
    def _parse_llm_dict(self, raw: dict, *, content_id: str | None) -> ContentDiscoveryProfile:
        ...
        conf_src = (
            raw.get("confidence_scores")
            if isinstance(raw.get("confidence_scores"), dict)
        ...
        profile = ContentDiscoveryProfile(
            content_id=content_id,
            title_suggestion=str(raw.get("title_suggestion") or raw.get("title") or "").strip()[
                :300
            ]
            or None,
            ...
            embedding_source_text=str(
                raw.get("embedding_source_text") or summary_short or ""
            ).strip()[:2000]
            or None,
        )
        ...
        *,
        ...
        if not settings.DISCOVERY_METADATA_ENABLED:
            ...
            if profile:
                ...
                profile.source = DiscoverySerialization.coerce_discovery_source(source) or None
                if speaker and (not profile.speaker):
                    profile.speaker = self._sanitize_speaker(speaker, transcript, trusted=True)
            return profile
        if not (transcript or "").strip():
            return None
