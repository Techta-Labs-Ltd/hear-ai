...
def resolve_magic_clean_lineage(
    ...
    *,
    ...
    """
    clean_url, clean_backend_id, clean_track_id = _validated_identity(
        submitted_url,
        backend_id,
        track_id,
        max_depth,
    )
    records = _scoped_jobs(jobs, clean_backend_id, clean_track_id)
    return _resolve_exact_url(
        clean_url,
...
def resolve_magic_clean_hash_alias(
    ...
    *,
    ...
    file_sha256 = _normalize_supplied_sha256(
        submitted_file_sha256,
        "submitted_file_sha256",
    )
    pcm_sha256 = _normalize_supplied_sha256(
    ...
    if not matches:
        return _new_source_resolution(clean_url)
    ...
    matched_methods = {
        method
        for _record, methods in matches.values()
...
def sha256_decoded_pcm(
    ...
    *,
    ...
    sample_rate, channels = _probe_audio_format(
        path,
        ffprobe_executable,
        timeout_seconds,
    )
    ...
    with tempfile.TemporaryFile() as stderr:
        ...
        try:
            ...
            process.wait()
            raise RuntimeError(
                "ffmpeg timed out while hashing decoded audio"
            ) from exc
...
def magic_clean_artifact_hashes(
    path: str | os.PathLike[str],
) -> tuple[str, str]:
    """Hash bytes and decoded PCM sequentially in one contained worker."""
    return sha256_file(path), sha256_decoded_pcm(path)
...
def _scoped_jobs(
    ...
    for candidate in jobs:
        ...
        root_hint = _clean_string(safe_options.get(MAGIC_CLEAN_ROOT_URL_KEY))
        parent_url = root_hint or _clean_string(getattr(candidate, "input_url", ""))
        records.append(
            _ScopedMagicCleanJob(
                job_id=_clean_string(getattr(candidate, "id", "")),
                output_url=extract_enhanced_audio_url(
                    getattr(candidate, "result_json", None)
                ),
                parent_url=parent_url,
                delivered_file_sha256=_normalize_persisted_sha256(
                    safe_options.get(MAGIC_CLEAN_DELIVERED_FILE_SHA256_KEY)
                ),
                delivered_pcm_sha256=_normalize_persisted_sha256(
    ...
    while True:
        ...
        if not candidates:
            if not lineage_job_ids:
                return _new_source_resolution(submitted_url)
            return MagicCleanLineageResolution(
...
def _normalize_persisted_sha256(value: object) -> str | None:
    ...
    return normalized if _is_sha256(normalized) else None
