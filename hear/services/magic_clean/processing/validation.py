...
MIN_MP3_DURATION_TOLERANCE_SECONDS = 0.030
...
CHANNEL_COLLAPSE_RELATIVE_THRESHOLD = 1e-4
...
class _DecodedAudioScan:
    ...
    def is_silent(self) -> bool:
        return (
            self.absolute_peak < SILENCE_PEAK_THRESHOLD
            and self.rms < SILENCE_RMS_THRESHOLD
        )
...
def validate_pcm(
    ...
    *,
    ...
    _validate_pcm_shape_and_values(waveform, label="Magic Clean PCM")
    if preserve_timeline:
        ...
        if waveform.shape[1] != expected_samples:
            raise AudioValidationError(
                "Magic Clean changed the PCM timeline without silence cutting: "
                f"expected={expected_samples}, actual={waveform.shape[1]}"
            )
...
def mp3_duration_tolerance_seconds(sample_rate: int) -> float:
    ...
    return max(
        MIN_MP3_DURATION_TOLERANCE_SECONDS,
...
def validate_delivered_audio(
    ...
    *,
    ...
    """
    source = _scan_decoded_audio(source_path, label="source")
    delivered = _scan_decoded_audio(output_path, label="delivered MP3")
    retained_reference = (
        _scan_decoded_audio(retained_reference_path, label="retained mix")
    ...
    if delivered.channels != source.channels:
        raise AudioValidationError(
            "Magic Clean changed the delivered channel count: "
            f"source={source.channels}, output={delivered.channels}"
        )
    if retained_reference is not None:
        if retained_reference.channels != source.channels:
            raise AudioValidationError(
                "Magic Clean changed the retained-mix channel count: "
                f"source={source.channels}, retained={retained_reference.channels}"
            )
        retained_duration_delta = abs(
            delivered.duration_seconds - retained_reference.duration_seconds
        )
        retained_tolerance = mp3_duration_tolerance_seconds(delivered.sample_rate)
        if retained_duration_delta > retained_tolerance + 1e-9:
            raise AudioValidationError(
                "Magic Clean delivered duration differs from the retained mix: "
                f"retained={retained_reference.duration_seconds:.6f}s, "
                f"output={delivered.duration_seconds:.6f}s, "
                f"allowance={retained_tolerance:.6f}s"
            )

    duration_delta = abs(delivered.duration_seconds - source.duration_seconds)
    duration_tolerance = mp3_duration_tolerance_seconds(delivered.sample_rate)
    if not cut_silence:
        if duration_delta > duration_tolerance + 1e-9:
            raise AudioValidationError(
                "Magic Clean encoded duration mismatch: "
                f"source={source.duration_seconds:.6f}s, "
                f"output={delivered.duration_seconds:.6f}s, "
                f"allowance={duration_tolerance:.6f}s"
            )
    else:
        if (
            retained_reference is not None
            and retained_reference.duration_seconds > source.duration_seconds + 1e-9
        ):
            raise AudioValidationError(
                "Magic Clean silence editing unexpectedly lengthened the retained mix"
            )
        if (
            delivered.duration_seconds
            > source.duration_seconds + duration_tolerance + 1e-9
        ):
            raise AudioValidationError(
                "Magic Clean silence editing unexpectedly lengthened the recording"
            )
        if expect_audible:
            minimum_retained_seconds = _minimum_retained_activity_seconds(source)
            if delivered.duration_seconds + duration_tolerance < minimum_retained_seconds:
                raise AudioValidationError(
                    "Magic Clean silence editing removed protected source activity: "
                    f"minimum={minimum_retained_seconds:.6f}s, "
                    f"output={delivered.duration_seconds:.6f}s"
                )
    ...
    if retained_reference is not None:
        if not retained_reference.is_silent and delivered.is_silent:
            raise AudioValidationError(
                "Magic Clean unexpectedly erased the retained mix"
            )
        retained_collapsed_channel = _collapsed_audible_channel(
            retained_reference,
            delivered,
        )
        if retained_collapsed_channel is not None:
            raise AudioValidationError(
                "Magic Clean unexpectedly erased a retained-mix channel: "
                f"channel={retained_collapsed_channel + 1}"
            )
    ...
    if expect_audible:
        collapsed_channel = _collapsed_audible_channel(source, delivered)
        if collapsed_channel is not None:
            raise AudioValidationError(
                "Magic Clean unexpectedly erased an audible source channel: "
                f"channel={collapsed_channel + 1}"
            )
    ...
    if delivered.clipped_sample_count:
        raise AudioValidationError(
            "Magic Clean artifact contains clipped decoded samples: "
            f"count={delivered.clipped_sample_count}"
        )
    ...
    if delivered.is_silent:
        ...
        peak_db = float(
            metrics.compute_true_peak_db(
                delivered.test_waveform,
                delivered.sample_rate,
            )
        )
        lufs = float(metrics.compute_lufs(delivered.test_waveform))
        snr = _snr_from_frame_powers(delivered.frame_powers)
        score = float(
            metrics.compute_quality_score(
                snr,
                False,
                lufs,
                snr_available=snr != 0.0,
            )
        )
    else:
        lufs, peak_db = _measure_decoded_loudness_and_true_peak(output_path)
        snr = _snr_from_frame_powers(delivered.frame_powers)
        score = float(
            metrics.compute_quality_score(
                snr,
    ...
    if peak_db > TRUE_PEAK_CEILING_DBTP + 1e-6:
        raise AudioValidationError(
            "Magic Clean artifact exceeds the delivered true-peak ceiling: "
            f"measured={peak_db:.2f} dBTP, ceiling={TRUE_PEAK_CEILING_DBTP:.2f} dBTP"
        )
...
def _scan_decoded_audio(path: str, *, label: str) -> _DecodedAudioScan:
    if not os.path.isfile(path):
        return _scan_injected_tensor(path, label=label)

    sample_rate, channels = _probe_audio_format(path, label=label)
    if channels not in {1, 2}:
        raise AudioValidationError(
            f"Decoded {label} audio must contain one or two channels"
        )
            ...
            while True:
                ...
                for channel in range(channels):
                    ...
                    channel_absolute_peaks[channel] = max(
                        channel_absolute_peaks[channel],
                ...
                complete_power_values = (
                    power_values.size // values_per_power_frame
                ) * values_per_power_frame
    ...
    if power_remainder.size:
        frame_power_parts.append(
            np.array([np.mean(power_remainder.astype(np.float64) ** 2)])
        )
    frame_powers = (
...
def _scan_injected_tensor(path: str, *, label: str) -> _DecodedAudioScan:
    ...
    _validate_pcm_shape_and_values(waveform, label=f"Decoded {label}")
    frame_samples = max(1, round(sample_rate * 0.02))
    frame_count = waveform.shape[1] // frame_samples
    if frame_count:
        core = waveform[:, : frame_count * frame_samples]
        powers = core.square().reshape(
            waveform.shape[0],
            frame_count,
            frame_samples,
        )
...
def _collapsed_audible_channel(
    source: _DecodedAudioScan,
    delivered: _DecodedAudioScan,
) -> int | None:
    ...
    for channel, (source_peak, source_rms, delivered_peak, delivered_rms) in enumerate(
        ...
        peak_collapsed = peak_is_audible and delivered_peak < max(
            ...
            source_peak * CHANNEL_COLLAPSE_RELATIVE_THRESHOLD,
        )
        rms_collapsed = rms_is_audible and delivered_rms < max(
            SILENCE_RMS_THRESHOLD,
            source_rms * CHANNEL_COLLAPSE_RELATIVE_THRESHOLD,
        )
...
def _measure_decoded_loudness_and_true_peak(path: str) -> tuple[float, float]:
    ...
    # EBU R128 has no integrated-loudness value for clips shorter than its
    # gating window. FFmpeg reports that one legitimate case as ``-inf`` even
    # though its true-peak measurement is valid. Preserve a finite unavailable
    # sentinel while continuing to reject NaN, +inf, or an invalid true peak.
    if lufs == -math.inf:
        lufs = -99.0
    if not math.isfinite(lufs) or not math.isfinite(true_peak):
        raise AudioValidationError(
            "Delivered loudness or true-peak measurement is non-finite"
        )
...
def _minimum_retained_activity_seconds(source: _DecodedAudioScan) -> float:
    ...
    if low_reference / (high_reference + 1e-12) > 0.65:
        # The silence editor deliberately leaves continuous or uniformly noisy
        # material untouched, so the delivered artifact must retain its length.
        return source.duration_seconds

    threshold = max(
        math.sqrt(max(low_reference, 1e-12) * high_reference),
        high_reference * 0.0158,
    )
    protective_threshold = min(
        threshold,
        low_reference * 1.10 + max(high_reference * 1e-4, 1e-7),
    )
