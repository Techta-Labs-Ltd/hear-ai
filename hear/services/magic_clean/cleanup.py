...
from hear.core.storage import storage_for_job
        ...
        for job in jobs:
            ...
            if isinstance(not_before_value, str) and not_before_value.strip():
                try:
                    ...
                    job.job_options = {
                        **options,
                        "magic_clean_cleanup_tombstone": _updated_failed_tombstone(
                            tombstone,
                            now=now,
                            error_type="InvalidCleanupNotBefore",
                        ),
                    }
                    continue
            ...
            # A retry writes to the same compatibility key. Never let an older
            # cleanup record race an active writer or a different run owner.
            if status in ACTIVE_JOB_STATUSES:
                continue
            if (
                tombstone_run_id
                and current_run_id
                and tombstone_run_id != current_run_id
            ):
                failed += 1
                job.job_options = {
                    **options,
                    "magic_clean_cleanup_tombstone": _updated_failed_tombstone(
                        tombstone,
                        now=now,
                        error_type="ArtifactOwnershipMismatch",
                    ),
                }
                continue

            if status == "completed":
                authoritative_key = _result_artifact_key(
                    getattr(job, "result_json", None)
                )
                if authoritative_key == key:
                    # Completion superseded the failed cleanup attempt. The
                    # key is now authoritative and must not be deleted.
                    ...
                    job.job_options = {
                        **options,
                        "magic_clean_cleanup_tombstone": _updated_failed_tombstone(
                            tombstone,
                            now=now,
                            error_type="CompletedArtifactUnknown",
                        ),
                    }
                    continue

            if status not in TERMINAL_JOB_STATUSES:
                # Unknown/legacy non-terminal states are protected by default.
                continue
            try:
                ...
                logger.error(
                    "Magic Clean cleanup reconciliation failed for job=%s",
                    job.id,
                )
            else:
                deleted += 1
