...
SUPPORTED_MP3_SAMPLE_RATES = (8000, 11025, 12000, 16000, 22050, 24000, 32000, 44100, 48000)
...
class StreamingAudioCleaner:
    ...
    def clean_file_streaming(
        ...
        *,
        ...
        work_dir = TempWorkspace.hear_temp_standalone_dir("magic_clean_stream")
        ...
        completed = False
        try:
            StreamingAudioCleaner._raise_if_cancelled(cancel_event)
            started = time.perf_counter()
            StreamingAudioCleaner._decode_lossless(input_path, decoded_path)
            StreamingAudioCleaner._raise_if_cancelled(cancel_event)
            stage_times["decode"] = elapsed_seconds(started)
            ...
            started = time.perf_counter()
            chunks = StreamingAudioCleaner._process_decoded_file(
                pipeline,
                decoded_path,
                processed_path,
                device=device,
            ...
            StreamingAudioCleaner._raise_if_cancelled(cancel_event)
            stage_times["enhance_and_stitch"] = elapsed_seconds(started)
            ...
            if output_samples != input_samples:
                raise RuntimeError(
                    f"Magic Clean context stitching changed the timeline: expected={input_samples}, actual={output_samples}"
                )
            if cut_silence:
                StreamingAudioCleaner._raise_if_cancelled(cancel_event)
                started = time.perf_counter()
                output_samples = StreamingAudioCleaner._apply_global_silence_edit(
                    pipeline, decoded_path, processed_path, silence_path, device=device, mode=mode
                )
                StreamingAudioCleaner._raise_if_cancelled(cancel_event)
                os.replace(silence_path, processed_path)
                stage_times["silence_edit"] = elapsed_seconds(started)
            StreamingAudioCleaner._raise_if_cancelled(cancel_event)
            started = time.perf_counter()
            measurements = StreamingAudioCleaner._measure_loudness(processed_path)
            StreamingAudioCleaner._raise_if_cancelled(cancel_event)
            stage_times["measure"] = elapsed_seconds(started)
            started = time.perf_counter()
            delivery_sample_rate = StreamingAudioCleaner._mp3_delivery_sample_rate(sample_rate)
            delivered_measurements = StreamingAudioCleaner._encode_with_true_peak_guard(
                processed_path,
                temporary_output,
                measurements,
            ...
            stage_times["master_and_encode"] = elapsed_seconds(started)
            ...
            source_probe = StreamingAudioCleaner._probe(input_path)
            output_probe = StreamingAudioCleaner._probe(output_path)
            integrated_lufs = StreamingAudioCleaner._finite_float(
                delivered_measurements.get("input_i"), default=-99.0
            )
            delivered_peak = StreamingAudioCleaner._finite_float(
                delivered_measurements.get("input_tp"), default=-99.0
            )
            ...
            for path in (temporary_output, decoded_path, processed_path, silence_path):
                try:
                    os.unlink(path)
                except OSError:
                    pass
            ...
            if not completed and validation_reference_path is not None:
                try:
                    ...
                    pass
            TempWorkspace.drop_temp_standalone(work_dir)

    @staticmethod
    def _decode_lossless(input_path: str, decoded_path: str) -> None:
        StreamingAudioCleaner._run(
            [
                "ffmpeg",
                "-nostdin",
                "-y",
        ...
        with sf.SoundFile(decoded_path) as source:
            ...
            with sf.SoundFile(
                ...
                for core_start in range(0, total_samples, core_samples):
                    ...
                    raw = source.read(window_end - window_start, dtype="float32", always_2d=True)
                    waveform = torch.from_numpy(raw.T.copy()).to(device)
                    cleaned = pipeline.process(waveform, sample_rate, mode, levels, False, False)
                    StreamingAudioCleaner._raise_if_cancelled(cancel_event)
    ...
    def _apply_global_silence_edit(
        ...
        *,
        ...
        edited = pipeline.finalise(
            waveform.to(device), sample_rate, mode, cut_silence=True, master=False
        )
    ...
    def _measure_loudness(path: str) -> dict[str, str]:
        completed = StreamingAudioCleaner._run(
            [
                "ffmpeg",
                "-nostdin",
                "-hide_banner",
    ...
    def _encode_normalised(
        ...
        *,
        ...
        input_lufs = StreamingAudioCleaner._finite_float(measurements.get("input_i"), default=-99.0)
        gain_db = StreamingAudioCleaner._bounded_master_gain_db(input_lufs)
        filters = [
            f"volume={gain_db:.6f}dB,alimiter=limit={PRE_ENCODE_PEAK_LIMIT:.8f}:level=false:latency=true"
        ]
    ...
    def _encode_with_true_peak_guard(
        ...
        *,
        ...
        for attempt in range(MAX_CODEC_TRUE_PEAK_REENCODES + 1):
            StreamingAudioCleaner._encode_normalised(
                input_path,
                output_path,
                measurements,
                sample_rate=sample_rate,
            ...
            delivered = StreamingAudioCleaner._measure_loudness(output_path)
            try:
    ...
    def _run(command: list[str]) -> subprocess.CompletedProcess[str]:
        return subprocess.run(
            command, capture_output=True, check=True, text=True, timeout=FFMPEG_TIMEOUT_SECONDS
        )
