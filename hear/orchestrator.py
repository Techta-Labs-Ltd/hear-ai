...
from hear.models.database import AiJob, AiTrackJob, SessionLocal
from hear.models.discovery import coerce_discovery_source
from hear.models.stages import get_description, get_label, get_stage
...
def _job_change_intervals(candidate: object) -> tuple[tuple[float, float], ...]:
    ...
    if isinstance(custom_tags, dict):
        intervals = _change_intervals(custom_tags.get("changes"))
        if intervals:
            return intervals

    result_json = getattr(candidate, "result_json", None)
    if isinstance(result_json, dict):
        return _change_intervals(result_json.get("segments"))
    return ()
...
def resolve_reconstruction_reference_url(
    ...
    *,
    ...
    requested_intervals = _change_intervals(changes)
    parent_by_output: dict[
        str,
    ...
    for candidate in jobs:
        ...
        if str(getattr(candidate, "status", "") or "") != "completed":
            continue
        candidate_type = str(getattr(candidate, "job_type", "") or "").replace(
            "-", "_"
        )
        ...
        result_json = getattr(candidate, "result_json", None)
        segment_outputs.update(_reconstruction_segment_urls(result_json))
        output_url = _reconstruction_rebuilt_url(result_json)
        if not output_url:
            continue
        ...
        parent_url = root_hint or str(
            getattr(candidate, "input_url", "") or ""
        ).strip()
        if not parent_url:
            continue

        parent = (parent_url, _job_change_intervals(candidate))
    ...
    for depth in range(max_depth):
        ...
        resolved_parent: tuple[str, tuple[tuple[float, float], ...]] | None = (
            parent_by_output[current_url]
        )
        ...
        if not requested_intervals or requested_intervals != parent_intervals:
            raise ValueError(
                "chained reconstruction must retry the same original intervals"
            )
...
class Orchestrator:
    ...
    def _pending_job(self, job_id: str, run_id: str) -> PendingJob | None:
        ...
        try:
            ...
            if job is None or job.status != "queued":
                return None
            if (
                (job.job_type or "pipeline") == "magic_clean"
                and not self._queued_magic_clean_storage_is_ready(db, job)
            ):
                return None
    ...
    def _queued_magic_clean_storage_is_ready(self, db, job: AiJob) -> bool:
        try:
            storage_context = decrypt_storage_context(
                job.storage_context_encrypted,
                require_active=False,
            )
        except StorageContextError:
            logger.error(
                "Queued Magic Clean job %s has no usable storage context; "
                "refusing scheduling",
                job.id,
            )
        ...
        if self._magic_clean_storage_context_has_lifetime(storage_context):
            return True
        logger.info(
            "Queued Magic Clean job %s is waiting for refreshed storage credentials",
            job.id,
        )
        self._mark_queued_magic_clean_storage_refresh_required(db, job)
        return False
    ...
    def _mark_queued_magic_clean_storage_refresh_required(self, db, job: AiJob) -> None:
        ...
        previous_stage = job.current_stage or (
        ...
        self._push_event(
            job.id,
            self._magic_clean_storage_refresh_event(
                job,
    ...
    def _start_scheduled_run(self, pending: PendingJob) -> asyncio.Task[None]:
        task = asyncio.create_task(
            self._run_scheduled(pending),
            name=f"hear-job:{pending.job_id}:{pending.run_id}",
        )
    ...
    def _finish_scheduled_run(
        ...
        # wrapper's ``finally``. The done callback calls this same idempotent
        # cleanup so scheduler capacity and the task registry cannot leak.
    ...
    async def _run_scheduled(self, pending: PendingJob) -> None:
        ...
        try:
            # A job may wait in the fair scheduler long enough for its storage
            # reserve to become unsafe. Recheck before taking a processing slot;
            # the row-locked claim below repeats this check authoritatively.
    ...
    async def cancel(self, job_id: str) -> bool:
        ...
        try:
            ...
            if job.status not in TERMINAL_STATUSES:
                ...
                refreshed_options = self._magic_clean_cancellation_options(
                    job,
                    cancelled_at_utc,
                )
                ...
                cancelled = (
                    ...
                    .filter(
                        AiJob.id == job_id,
                        AiJob.status.notin_(tuple(TERMINAL_STATUSES)),
                    )
                    .update(
                        cancel_values,
    ...
    @staticmethod
    def _magic_clean_cancellation_options(
        job: AiJob,
        ...
        refreshed_tombstone = {
            **tombstone,
            ...
            "not_before": (
                cancelled_at
                + timedelta(seconds=settings.MAGIC_CLEAN_CLEANUP_GRACE_SECONDS)
            ).isoformat(),
        }
        return {
            **options,
            "magic_clean_cleanup_tombstone": refreshed_tombstone,
        }
    ...
    async def _process(self, job_id: str, run_id: str):
        ...
        try:
            ...
            job_storage = storage_for_job(job)
            ...
            if (
                ...
                try:
                    ...
                    pass
                try:
                    parked_event = await self._park_magic_clean_for_storage_refresh(
                        job_id,
                        run_id,
                    )
                except Exception:
                    logger.exception(
                        "Could not park Magic Clean job %s for credential refresh",
                        job_id,
                    )
                else:
    ...
    @staticmethod
    def _magic_clean_storage_refresh_event(
        job: AiJob,
        *,
        ...
        return {
            ...
            "description": (
                "Magic Clean is paused until storage credentials have the "
    ...
    async def _park_magic_clean_for_storage_refresh(
        self,
        job_id: str,
        run_id: str,
    ) -> dict | None:
        park_db = SessionLocal()
        try:
            ...
            if track_job is not None:
                    ...
                    .update(
                        {
                            ...
                            AiTrackJob.updated_at: datetime.now(UTC).replace(
                                tzinfo=None
                            ),
                        },
                        synchronize_session=False,
                ...
                if track_parked != 1:
                    park_db.rollback()
                    return None
            await commit_with_retry(park_db)
            return self._magic_clean_storage_refresh_event(
                job,
                previous_stage=previous_stage,
            )
    ...
    def _sanitize_error(error: Exception) -> str:
        ...
        storage_capacity_errors = (
            "storage cap", "storage capacity", "quota exceeded", "cap exceeded"
        )
        if any(key in msg for key in storage_capacity_errors):
            return (
                "Cloud storage is full. Free up space or increase your storage cap, "
                "then try again."
            )
    ...
    def _magic_clean_jobs_outside_scope(db, job: AiJob) -> list[AiJob]:
        ...
        return [
            ...
            if candidate.backend_id != job.backend_id
            or candidate.track_id != job.track_id
        ]
    ...
    def _reject_cross_scope_magic_clean_match(
        ...
        *,
        ...
        for candidate in candidates:
            if submitted_url and extract_enhanced_audio_url(candidate.result_json) == submitted_url:
                raise MagicCleanLineageError(
                    "Known Magic Clean output belongs to a different track scope"
                )
            options = (
                candidate.job_options
                if isinstance(candidate.job_options, dict)
                else {}
            )
            if submitted_file_sha256 and str(
                options.get(MAGIC_CLEAN_DELIVERED_FILE_SHA256_KEY) or ""
            ) == submitted_file_sha256:
                raise MagicCleanLineageError(
                    "Known Magic Clean bytes belong to a different track scope"
                )
            if submitted_pcm_sha256 and str(
                options.get(MAGIC_CLEAN_DELIVERED_PCM_SHA256_KEY) or ""
    ...
    async def _prepare_magic_clean_source(
        ...
        *,
        ...
        persisted_engine = str(
            options.get(MAGIC_CLEAN_ENGINE_REVISION_KEY) or ""
        ).strip()
        ...
        expected_file_hash = str(
            options.get(MAGIC_CLEAN_SOURCE_FILE_SHA256_KEY) or ""
        ).strip() or None
        expected_pcm_hash = str(
            options.get(MAGIC_CLEAN_SOURCE_PCM_SHA256_KEY) or ""
        ).strip() or None
        if persisted_root:
            ...
            parent_job_id = str(
                options.get(MAGIC_CLEAN_PARENT_JOB_ID_KEY) or ""
            ).strip()
            matched_by = "persisted_retry"
            if parent_job_id and not (expected_file_hash and expected_pcm_hash):
                recovered_file_hash, recovered_pcm_hash = (
                    self._known_magic_clean_source_hashes(
                        lineage_jobs,
                        {parent_job_id},
                    )
                )
            ...
            parent_job_id = (
                resolution.lineage_job_ids[0]
                if resolution.lineage_job_ids
                else ""
            )
            matched_by = resolution.matched_by
            expected_ids = set(resolution.lineage_job_ids) | set(
                resolution.matched_job_ids
            )
            expected_file_hash, expected_pcm_hash = (
                self._known_magic_clean_source_hashes(
                    lineage_jobs,
                    expected_ids,
                )
        ...
        file_hash, pcm_hash = await run_blocking_to_completion(
            partial(magic_clean_artifact_hashes, audio_path)
        )
        self._reject_cross_scope_magic_clean_match(
            outside_scope_jobs,
        ...
        if not persisted_root and root_url == submitted_url:
            alias = resolve_magic_clean_hash_alias(
                submitted_url,
                backend_id=str(job.backend_id or ""),
                track_id=track_id,
                jobs=lineage_jobs,
            ...
            if alias.is_known_derivative:
                ...
                parent_job_id = (
                    alias.lineage_job_ids[0]
                    if alias.lineage_job_ids
                    else alias.matched_job_ids[0]
                )
    ...
    def _magic_clean_reuse_candidate(
        ...
        for candidate in lineage_jobs:
            ...
            if (
                ...
                or options.get(MAGIC_CLEAN_ENGINE_REVISION_KEY)
            ...
            if not isinstance(quality, dict):
                continue
            if not extract_enhanced_audio_url(result):
                continue
            delivered_file_hash = str(
                options.get(MAGIC_CLEAN_DELIVERED_FILE_SHA256_KEY) or ""
            )
            delivered_pcm_hash = str(
                options.get(MAGIC_CLEAN_DELIVERED_PCM_SHA256_KEY) or ""
            )
    ...
    async def _try_reuse_magic_clean_artifact(
        ...
        prior_url = extract_enhanced_audio_url(candidate.result_json)
        expected_file_hash = str(
            options.get(MAGIC_CLEAN_DELIVERED_FILE_SHA256_KEY) or ""
        )
        expected_pcm_hash = str(
        ...
        try:
            ...
            reused_path = await download_audio(
                prior_url,
                suffix=".mp3",
                db=db,
                job_id=job.id,
            ...
            enhanced_url = await run_blocking_to_completion(
                partial(
                    storage.upload_file,
                    reused_path,
                    remote_key,
            ...
            return {
                ...
                "source_file_sha256": source_metadata[
                    MAGIC_CLEAN_SOURCE_FILE_SHA256_KEY
                ],
                "source_pcm_sha256": source_metadata[
                    MAGIC_CLEAN_SOURCE_PCM_SHA256_KEY
                ],
            ...
            if reused_path:
                drop_temp_standalone(reused_path)
        ...
        *,
        ...
        tombstone = {
            ...
            "delivered_file_sha256": str(
                enhancement.get("delivered_file_sha256") or ""
            ),
            "reason": reason,
        ...
        # Use an independent, row-locked session. Completion can fail
        # ambiguously after its transaction is sent, and cancellation may race
        # the provisional write that makes a future artifact recoverable.
        ...
        try:
            persisted_job = (
                ...
                .filter(
                    AiJob.id == job.id,
                    AiJob.run_id == getattr(job, "run_id", None),
                )
                .with_for_update()
                .first()
            )
            ...
            # Keep the caller's attached object in sync so later stage commits
            # cannot overwrite the independently persisted cleanup intent.
            job.job_options = dict(persisted_options)
            await run_awaitable_to_completion(commit_with_retry(cleanup_db))
        ...
        *,
        ...
        if defer_deletion:
            await self._record_magic_clean_cleanup_tombstone(
                ...
                enhancement,
                reason=reason,
                error_type="DeferredOwnershipCheck",
            )
            return
        try:
            await run_blocking_to_completion(partial(storage.delete_object, key))
        except asyncio.CancelledError:
            await self._record_magic_clean_cleanup_tombstone(
                job,
                enhancement,
            ...
            logger.error(
                "Magic Clean artifact cleanup requires reconciliation for job=%s",
                job.id,
            )
    ...
    async def _process_magic_clean(self, job: AiJob, track_job: AiTrackJob, db):
        ...
        if not await self._set_stage(db, job, track_job, "downloading", levels):
            return
        track = self._track_from_job(job)
        audio_path, source_metadata, download_seconds = (
            await self._prepare_magic_clean_source(
                db,
                job,
                submitted_url=submitted_url,
                track_id=track.track_id,
        ...
        # Persist cleanup ownership before either upload path can write. If the
        # orchestrator dies after the actor publishes but before completion,
        # recovery can still delete the uncommitted compatibility-key object.
        await self._record_magic_clean_cleanup_tombstone(
            job,
            expected_artifact,
            reason="upload_pending",
            error_type="ProvisionalArtifact",
        )
        reuse_candidate = self._magic_clean_reuse_candidate(
            self._magic_clean_lineage_jobs(db, job),
            source_metadata,
            levels,
        )
        enhancement = None
        ...
        if enhancement is None:
            ...
            try:
                ...
                enhancement = await run_awaitable_to_completion(
                    remote_response,
                    on_cancel=cancel_remote if callable(cancel_remote) else None,
                )
            except asyncio.CancelledError:
                await self._cleanup_magic_clean_artifact(
                    db,
                    ...
                    storage,
                    expected_artifact,
                    reason="cancelled_during_remote_processing",
                )
                raise
            except Exception:
                await self._cleanup_magic_clean_artifact(
                    ...
                    storage,
                    expected_artifact,
                    reason="remote_processing_failed",
                )
                raise
        try:
            ...
            await self._cleanup_magic_clean_artifact(
                ...
                storage,
                expected_artifact,
                reason="invalid_remote_result",
            )
            raise
        try:
            mixing_ready = await self._set_stage(
                db,
                job,
                track_job,
                "mixing",
            ...
            await self._cleanup_magic_clean_artifact(
                ...
                storage,
                enhancement,
                reason="cancelled_during_mixing_transition",
            )
            raise
        except Exception:
            await self._cleanup_magic_clean_artifact(
                ...
                storage,
                enhancement,
                reason="mixing_transition_failed",
            )
            raise
        if not mixing_ready:
            await self._cleanup_magic_clean_artifact(
                ...
                storage,
                enhancement,
                reason="cancelled_after_upload",
            )
            return
        drop_temp_standalone(audio_path)
        try:
            finalizing_ready = await self._set_stage(
                db,
                job,
                track_job,
                "finalizing",
            ...
            await self._cleanup_magic_clean_artifact(
                ...
                storage,
                enhancement,
                reason="cancelled_during_finalizing_transition",
            )
            raise
        except Exception:
            await self._cleanup_magic_clean_artifact(
                ...
                storage,
                enhancement,
                reason="finalizing_transition_failed",
            )
            raise
        ...
        options.update(
            {
                MAGIC_CLEAN_DELIVERED_FILE_SHA256_KEY: enhancement[
                    "delivered_file_sha256"
                ],
                MAGIC_CLEAN_DELIVERED_PCM_SHA256_KEY: enhancement[
                    "delivered_pcm_sha256"
                ],
        ...
        job.job_options = options
        stage_times = {
            "downloading": download_seconds,
            **enhancement["stage_times"],
        }
        result = {
            "job_id": job.id,
            "run_id": job.run_id,
    ...
    def _resolve_reconstruction_reference_url(
        ...
        requested_intervals = _change_intervals(changes)
        persisted_root = str(
            options.get("voice_reference_audio_url") or ""
        ).strip()
        persisted_intervals = _change_intervals(
            options.get("voice_reference_intervals")
        )
        if (
            persisted_root
            and requested_intervals
            and requested_intervals == persisted_intervals
        ):
        ...
        resolved_url, hops = resolve_reconstruction_reference_url(
            submitted_url,
            backend_id=str(job.backend_id or ""),
            track_id=track_id,
            changes=changes,
    ...
    async def _download_reconstruction_root(
        ...
        *,
        ...
        try:
            reference_path = await download_audio(
                reference_url,
                suffix=".wav",
                db=db,
                job_id=job.id,
            ...
            logger.error(
                "Immutable reconstruction root is unavailable "
                "for track=%s; refusing recursive reconstruction",
                track_id,
            )
            raise

        _recon_logger.info(
            "REFERENCE_LINEAGE | track=%s | hops=%d | immutable=true",
            track_id,
            lineage_hops,
        )
    ...
    async def _process_reconstruct(self, job: AiJob, track_job: AiTrackJob, db):
        ...
        if not changes:
            raise ValueError(
                "reconstruct requires changes[] with "
                "segment_start/segment_end/new_text"
            )
        track = self._track_from_job(job)
        ...
        immutable_root_path = (
            await self._download_reconstruction_root(
                db,
                job,
                track_id=track.track_id,
        ...
        voice_reference_audio_path = (
            reconstruction_audio_path if same_speaker else None
        )
